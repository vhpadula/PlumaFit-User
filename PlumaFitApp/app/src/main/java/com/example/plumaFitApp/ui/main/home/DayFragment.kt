...
//        }
//    }
fun generateMockMeasurements(): MeasurementDetailResponse {
    val measurements = mutableListOf<MeasurementDetails>()
    val details = listOf(
        MeasurementDetails("90min", "Exercício", "Depois do Café", "Observation 1"),
        MeasurementDetails("Bom", "Emoção", "No Almoço","Observation 2") ,
        MeasurementDetails("2", "Refeição", "Pão com Ovo","Observation 2")
    )

    for (detail in details) {
        measurements.add(detail)
    }
    return MeasurementDetailResponse(measurements)
}
    ...
    private fun setAdapter() {
        ...
        adapter.loadData(generateMockMeasurements())
        binding.run {
